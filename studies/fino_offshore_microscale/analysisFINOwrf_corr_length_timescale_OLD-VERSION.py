...
def spatialCorrelation2D (ds, x0, y0, ti=None, tf=None, dateref=None):
    ...
    print(f'Performing spatial correlation wrt to point ({ds.isel(x=iNearest).x.values}, '           f'{ds.isel(y=jNearest).y.values}), between {ti} and {tf}.', end='\r', flush=True)
...
def performCorrelationSubDomain (ds, nSubDomainsX, nSubDomainsY, outputnc=None, ti=133200, tf=133200+3*3600, window=1800, inc=600):
    ...
    print(xSubDom, ySubDom)
    print(nSubDomainsX + 1, nSubDomainsY + 1 )
    vlist = []
    while ti+window <= tf:
        print('blarp')
        ...
        for i, _ in enumerate(xSubDom[:-1]):
            for j, _ in enumerate(ySubDom[:-1]):
                ...
                print(xSubDom[i],xSubDom[i+1]-1, ySubDom[j],ySubDom[j+1]-1)
                ...
                print("DIMS: ", v_curr.dims)
                print("COORDS: ", v_curr.coords)
                try:
                    print("VARIABLES: ", v_curr.data_vars)
                except:
                    print("ERROR printing data_vars, moving on to the next file")
                vlist.append(v_curr)
        ti = ti + inc
...
print(ds80m.data_vars)
print(ds80m.u, ds80m.v)
#ds80m['wspd'], ds80m['wdir'] = calc_wind(ds80m)
